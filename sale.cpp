...
// Misc helpers
void pauseReturn();
double applySeasonalSale(const string &dateStr, int &discountPercent) {
    // Extract month name or number from the input (e.g., "December 25, 2025" or "12/25/25")
    string dateLower = dateStr;
    transform(dateLower.begin(), dateLower.end(), dateLower.begin(), ::tolower);

    discountPercent = 0;

    if (dateLower.find("dec") != string::npos || dateLower.find("/12") != string::npos) {
        discountPercent = 20;
        cout << "\nChristmas Sale Active! You get 20% off your total stay!\n";
    } 
    else if (dateLower.find("feb") != string::npos || dateLower.find("/02") != string::npos) {
        discountPercent = 10;
        cout << "\nValentine's Promo! You get 10% off your total stay!\n";
    } 
    else if (dateLower.find("apr") != string::npos || dateLower.find("/04") != string::npos ||
             dateLower.find("may") != string::npos || dateLower.find("/05") != string::npos) {
        discountPercent = 15;
        cout << "\nSummer Sale! Enjoy 15% off your booking!\n";
    }
